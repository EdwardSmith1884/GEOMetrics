...
norms = norm(mean=[0.485, 0.456, 0.406],
...
def load_initial( obj='scripts/386.obj'):
	obj = ObjLoader(obj)
...
def adj_init(faces,num_verts):
	...
	adj_orig = (adj + sp.eye(adj.shape[0])).todense()
...
def calc_face_list(faces):
	...
	face_list = [[0,0,0] for i in range(len(faces))]
	...
	def __init__(self, Img_location, Mesh_loction, Sample_location):
		...
		for n in tqdm(names):
			if os.path.exists(self.Sample_location + n  +'.mat'):
					self.names.append(n)
		self.rot1 = torch.FloatTensor(rotation_matrix( [1,0,0], 1.2)).cuda()
		self.rot2 = torch.FloatTensor(rotation_matrix( [0,1,0], 1.2)).cuda()
		self.rot3 = torch.FloatTensor(rotation_matrix( [0,0,1], 1.2)).cuda()


		print 'The number of objects found : '  + str(len(self.names))
	def load_batch(self, size, voxels = True, meshes = True, images = True, odms = False,  number = None, ordered = False, named = None, set = 'train'):
		batch_names = []
		namer = []
		latent = []
...
class Voxel_loader(object):
	def __init__(self, Img_location, Mesh_loction,Voxel_location):
		...
		print 'The number of objects found : '  + str(len(self.names))
	def load_batch(self, size, voxels = True, meshes = True, images = False, odms = False,  number = None, ordered = False, named = None, set = 'train'):
		batch_names = []
		namer = []
		faces = []
		for i in range(size):
			...
			if meshes:
				...
				# meshes = glob(self.Mesh_loction + '0_' + obj + '*')
				mesh_info  =  sio.loadmat(self.Mesh_loction +obj +  '.mat'  )
				verts = mesh_info['verts']
				verts= torch.FloatTensor(np.array(verts)).cuda()
				...
				batch_norms.append(torch.FloatTensor(np.array(normals)).cuda())
		...
		batch = {'names':np.array(batch_names)}
		if odms:
			batch['odms'] = torch.FloatTensor(np.array(faces)).cuda()
		elif voxels:
			batch['voxels'] = torch.FloatTensor(np.array(batch_voxels)).cuda()
		if meshes:
			batch['verts'] =  batch_verts
			batch['norms'] =  batch_norms
...
def calc_curve( verts, info, degree = 70):
	eps = .0000001
	...
	splitting_faces  = np.where(curvature_proxi*180/np.pi  >degree)[0]
...
def split_info( info, verts, features,  splitting_face_list_indecies ,number ):
	...
	# compute matrix for caluculating new feature vecotr and new adj matrix
	adj = info['adj_orig']
	faces = np.array(info['faces'] )
	face_list = torch.LongTensor(np.array(info['face_list'])).cuda()



	splitting_face_list_values = torch.index_select(face_list, 0,  splitting_face_list_indecies )
	...
	unsplitting_face_list_values = torch.index_select(face_list, 0, torch.LongTensor(unsplitting_faces_list_indecies).cuda())


	splitting_faces_indecies = splitting_face_list_values[:,0,2]

	unsplitting_faces_indecies = unsplitting_face_list_values[:,0,2]



	new_faces_indecies_1 = np.arange(splitting_face_list_len).reshape(-1,1) + faces.shape[0]
	...
	new_positions = np.zeros((faces.shape[0], 3))
	...
	# new triangle 1
	splitting_connecting_face_1_1  = new_positions[splitting_face_list_values[:,0,0],splitting_face_list_values[:,0,1] ].reshape(-1,1,1)
	splitting_connecting_face_1_2  = new_faces_indecies_2.reshape(-1,1,1)
	splitting_connecting_face_1_3  = new_faces_indecies_3.reshape(-1,1,1)

	splitting_connecting_side_1_1 = splitting_face_list_values[:,0,1].reshape(-1,1,1)
	splitting_connecting_side_1_2 = np.zeros(splitting_face_list_len).reshape(-1,1,1)
	...
	split_faces = faces[splitting_faces_indecies]
	...
	faces = np.concatenate( (faces, new_face_1, new_face_2, new_face_3))

	new_adj = np.zeros((new_len, new_len))

	'v1 -> v1, x, y, z'


	v1_adj = np.zeros((v1.shape[0], new_len ))
	v1_repeat = np.arange(0,v1.shape[0])
	v1_places_1 = np.concatenate((v1_repeat, v1_repeat,v1_repeat,v1_repeat)).reshape(-1)
	v1_places_2 = np.concatenate((v1_inds, x_f, y_f, z_f)).reshape(-1)
	v1_adj[v1_places_1, v1_places_2] = 1

	new_adj[v1_inds.reshape((-1))] =  v1_adj
	new_adj[:vertex_count, :vertex_count] =   adj
	new_adj = np.maximum(new_adj, new_adj.T)
	adj_orig = np.array(new_adj)


	adj = normalize(new_adj)
	adj = torch.FloatTensor(adj).cuda()

	adj_info = {'adj' : adj, 'faces': faces, 'adj_orig': adj_orig, 'face_list' : new_splitting_face_list }
	features = verts[:,3:]
	verts = verts[:,:3]
...
def calc_edge( verts, info):
	...
	e2 = p3-p1
	e3 = p2-p3
	return (torch.sum(e1**2, 1).mean() + torch.sum(e2**2, 1).mean() + torch.sum(e3**2, 1).mean())/3.
